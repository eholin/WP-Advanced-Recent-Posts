...
function lptw_recent_posts_register_scripts() {
	...
	wp_enqueue_script( 'lptw-recent-posts-script', plugins_url( 'lptw-recent-posts.js', __FILE__ ), array(
		'jquery',
		'jquery-masonry',
	), FALSE, TRUE );
}
...
function lptw_recent_posts_options() {
	add_meta_box( 'lptw_recent_posts_options', __( 'Advanced Recent Posts Options', 'lptw_recent_posts_domain' ), 'lptw_recent_posts_options_box_content', 'post', 'normal', 'default' );
}
...
function lptw_display_recent_posts( $atts ) {
	...
	if ( strpos( $a['authors_id'], ',' ) !== FALSE ) {
		...
		$authors_id = (integer) $a['authors_id'];
	}
	...
	if ( $lptw_shortcode_query->have_posts() ) {
		...
		$prepared_args = lptw_prepare_args_for_js( $a, $array_append );

		$content = lptw_display_layout_header( $a['layout'], $rand_grid, $prepared_args );
		while ( $lptw_shortcode_query->have_posts() ){
			...
			if ( $post_subtitle != '' AND $a['show_subtitle'] == 'true' ) {
			...
			$element_style_args = [ ];
			if ( $a['width'] != '' AND $a['fluid_images'] != 'true' ) {
				$element_style_args['width'] = $a['width'] . 'px';
			}
			if ( $a['height'] != '' AND $a['fluid_images'] != 'true' ) {
				$element_style_args['height'] = $a['height'] . 'px';
			}

			if ( $a['columns'] > 1 AND $a['layout'] != 'grid-medium' ) {
				if ( ( $i % $a['columns'] ) == 0 ) {
			...
			} else if ( $a['columns'] == 1 && $a['layout'] != 'grid-medium' ) {
				$element_style_args['padding-bottom'] = $a['space_ver'] . 'px';
			} else {
 ...
 **/
function lptw_display_layout_header( $layout, $rand_grid, $args ) {
	$content = '';
	switch ( $layout ) {
		case 'basic':
		...
		case 'grid-medium':
			$data_str = '';
			if ( is_array( $args ) ) {
				$data_arr = lptw_array_to_html_data( $args );
				if ( is_array( $data_arr ) ) {
					$data_str = implode( ' ', $data_arr );
				}
			}
			$content = '<div class="lptw-container" id="lptw-grid-' . $rand_grid . '" ' . $data_str . '>';
			break;
	}
...
function lptw_prepare_args_for_js( $shortcode_args, $array_append ) {
	$js_keys = array(
		0 => 'fluid_images',
		1 => 'space_hor',
		2 => 'columns',
	...
	if ( is_array( $shortcode_args ) ) {
		foreach ( $shortcode_args as $data_key => $data_value ) {
			if ( in_array( $data_key, $js_keys ) ) {
				$js_data[ $data_key ] = $data_value;
			}
		}
	}

	if ( is_array( $array_append ) ) {
		$result_array = array_merge( $js_data, $array_append );
	} else {
		$result_array = $js_data;
	}

	return $result_array;
}

function lptw_array_to_html_data( $array ) {
	$datas = [ ];
	if ( is_array( $array ) ) {
		foreach ( $array as $data_key => $data_value ) {
			$datas[] = 'data-' . $data_key . '="' . $data_value . '"';
		}
	}
