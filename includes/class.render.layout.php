...
function render_post_date( $a, $post_date, $post_time ) {
	if ( $a['show_time'] == 'true' && $a['show_time_before'] == 'true' ) {
		$post_date_time = $post_time . ' ' . $post_date;
	} else if ( $a['show_time'] == 'true' && $a['show_time_before'] != 'true' ) {
...
function render_element_attributes( $object ) {
	...
	if ( ! empty( $object['class'] ) ) {
		$content .= ' ' . render_element_class( $object['class'] );
	}
	/* create style if exist */
	if ( ! empty( $object['style'] ) ) {
		$content .= ' ' . render_element_styles( $object['style'] );
	}
	/* create style if exist */
	if ( ! empty( $object['href'] ) ) {
		$content .= ' href="' . $object['href'] . '"';
	}
	/* create target if exist */
	if ( ! empty( $object['target'] ) ) {
		$content .= ' target="' . $object['target'] . '"';
	}
...
function render_article_objects( $container_name, $layout_objects ) {
	...
	foreach ( $layout_objects as $object ) {
		if ( $object['display'] == 'true' AND $object['container'] == $container_name ) {
			$content .= '<' . $object['tag'];
...
function render_article_part( $place, $layout_containers, $layout_objects ) {
	...
	foreach ( $layout_containers as $container ) {
		if ( $container['display'] == 'true' AND $container['place'] == $place ) {
